...
/** Saves an explicit human Chat entry as Capture. Candidate proposal is opt-in; Task creation is impossible here. */
export async function POST(request: NextRequest) {
  ...
  const parsed = parseCaptureBody(
    body && typeof body === 'object' && !Array.isArray(body)
      ? { ...(body as Record<string, unknown>), source: 'human' }
      : body,
  )
  if (!parsed) return Response.json({ error: 'BODY_INVALID' }, { status: 400, headers })

  try {
    const result = await context.service.capture(context.viewer, {
      ...parsed,
      source: 'human',
      evidenceRefs: [],
      unknowns: [],
