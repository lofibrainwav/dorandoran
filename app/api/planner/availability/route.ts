...
import { parseHouseholdMembership, resolveHouseholdTimeZone, resolveUniqueChildPersonId } from '@/lib/family-os'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [operational, local] = await Promise.all([
      childPersonId ? loadPrivateOperationalFamilyCalendarPerson({ personId: childPersonId, label: 'Jayden', now, timeZone, modules }) : null,
      privateEnabled ? loadPrivateCalendarOperatingPerson({ personId: 'person-jayden', label: 'Jayden', now, timeZone, modules }) : null,
    ])
