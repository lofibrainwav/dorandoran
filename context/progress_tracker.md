...
1. Implement the Drive API ports (`listFiles`/`readFile`) that Unit 37 takes by injection, and a local trigger that calls `runDriveOutboxSession`. Everything above them is done and tested: contract (33), plan/classify (34), text→record + `drive.readonly` (36), the run (37), the cursor (38), the cycle (39). **This step cannot be verified from a build seat** — it needs a session that can complete a real Google OAuth round trip against the `04_DORANDORAN_OUTBOX` folders. Read-only; no scheduler, no Drive write.
