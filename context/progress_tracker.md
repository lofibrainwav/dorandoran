...
- Cron (step 7) is **intentionally deferred**. The only real record in the outbox is the smoke record and it is already processed, so a scheduler would verify one thing: that an empty lane can be polled reliably. Operating model until there is real workload: manual run plus nightly reconcile.
