...
export function replanCalendarAwareBlocks(
  ...
  const activeBlock = blocks.find(b => b.isCurrent && b.source === 'oneblock' && !b.isCompleted)
  let startMinutes: number
  
  if (activeBlock) {
    // Start after the current active block ends (including its buffer)
    const activeEndMinutes = parseTimeToMinutes(activeBlock.endTime)
    startMinutes = activeEndMinutes + (activeBlock.bufferAfter || 0)
  } else {
    // Default to 9:00 AM for demo stability
    startMinutes = 9 * 60
  }
